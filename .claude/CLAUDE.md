...
6. **Submit for Review** — Run the external review API (REVIEWER_MODE=api-external, already set in .env):
   ...
   ```
   This calls our hosted review API (REVIEW_API_URL=https://review-api.eigenlabs.online). Use timeout:2400000. REVIEWER_MODE=api-external is already set in .env — do NOT change it to subagent or ensemble. Never use subagent or ensemble modes.
